...
const resolvers = {
  ...
  Mutation: {
    ...
    addGame(_, args) {
      let game = {
        ...args.game,
        id: Math.floor(Math.random() * 10000).toString(), // Generate Random number between 1-1000
      };
      _db.games.push(game);
    ...
    updateGame(_, args) {
      _db.games = _db.games.map((g) => {
        if (g.id === args.id) {
          return { ...g, ...args.edits }; // overwrite g with args.edits
        }
